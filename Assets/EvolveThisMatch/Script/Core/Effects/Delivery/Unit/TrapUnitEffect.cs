...
namespace EvolveThisMatch.Core
{
    public class TrapUnitEffect : UnitEffect
    {
        ...
        public override void Deliver(EffectContext effectContext, Unit casterUnit, Unit targetUnit)
        {
            ...
            if (_isInfinity)
            {
                casterUnit.GetAbility<EntitySpawnAbility>().SpawnTrap(_prefab, targetUnit.transform.position, (caster, target) => { Resolve(effectContext, caster, target); });
            }
            else
            {
                casterUnit.GetAbility<EntitySpawnAbility>().SpawnTrap(_prefab, targetUnit.transform.position, _duration, (caster, target) => { Resolve(effectContext, caster, target); });
            }
        }
