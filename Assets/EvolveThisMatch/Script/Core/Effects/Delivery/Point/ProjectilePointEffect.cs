...
namespace EvolveThisMatch.Core
{
    public class ProjectilePointEffect : PointEffect
    {
        ...
        public override void Deliver(EffectContext effectContext, Unit casterUnit, Vector3 targetVector)
        {
            ...
            switch (_rangeType)
            {
                case ENontargetProjectileRangeType.Straight:
                    SpawnStraightProjectiles(effectContext, casterUnit, direction, distance);
                    break;
                case ENontargetProjectileRangeType.Cone:
                    SpawnConeProjectiles(effectContext, casterUnit, direction, distance);
                    break;
            }
        }

        private void SpawnStraightProjectiles(EffectContext effectContext, Unit casterUnit, Vector3 direction, float distance)
        {
            Vector3 finalPosition = casterUnit.transform.position + direction * distance;
            SpawnProjectile(effectContext, casterUnit, finalPosition);
        }

        private void SpawnConeProjectiles(EffectContext effectContext, Unit casterUnit, Vector3 direction, float distance)
        {
        ...
        private void SpawnProjectile(EffectContext effectContext, Unit casterUnit, Vector3 finalPosition)
        {
            casterUnit.GetAbility<EntitySpawnAbility>().SpawnProjectile(_prefab, _spawnPoint, finalPosition, (caster, target) => { Resolve(effectContext, caster, target); });
        }
