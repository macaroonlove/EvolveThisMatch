...
namespace EvolveThisMatch.Core
{
    public class InstantPointEffect : PointEffect
    {
        ...
        public override void Deliver(EffectContext effectContext, Unit casterUnit, Vector3 targetVector)
        {
            ...
            switch (_rangeType)
            {
                case ERangeType.Circle:
                    GetTargetInCircle(effectContext, casterUnit, maxCount);
                    break;
                case ERangeType.Straight:
                    GetTargetStraight(effectContext, casterUnit, targetVector, maxCount);
                    break;
                case ERangeType.Cone:
                    GetTargetCone(effectContext, casterUnit, targetVector, maxCount);
                    break;
                case ERangeType.Line:
                    GetTargetGrid(effectContext, casterUnit, maxCount);
                    break;
                default:
                    GetAllTarget(effectContext, casterUnit);
                    break;
            }
        }

        private void GetAllTarget(EffectContext effectContext, Unit casterUnit)
        {
            var targets = casterUnit.GetAbility<FindTargetAbility>().FindAllTarget(_unitType);

            foreach (var target in targets)
            {
                Resolve(effectContext, casterUnit, target);

                ExecuteTargetFX(target);
            }
        }

        private void GetTargetInCircle(EffectContext effectContext, Unit casterUnit, int maxCount)
        {
        ...
        private void GetTargetGrid(EffectContext effectContext, Unit casterUnit, int maxCount)
        {
