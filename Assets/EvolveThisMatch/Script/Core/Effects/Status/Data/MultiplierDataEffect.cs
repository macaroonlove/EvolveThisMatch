...
{
    public abstract class MultiplierDataEffect : DataEffect<float>, IMutableValueBindingProvider
    {
        ...
        private string FormatDescription(float value)
        {
            if (value == 0)
            {
                return $"{GetTitle()}을(를) 상승·하락 시켜주세요.";
            }
            else if (value > 0)
            {
                return $"{GetTitle()}  {value * 100}% 상승";
            }
            else
            {
                return $"{GetTitle()}  {Mathf.Abs(value) * 100}% 하락";
            }
        }
    }
}