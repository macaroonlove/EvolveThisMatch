...
{
    public class UIAgentListCanvas_Formation : UIAgentListCanvas<UIAgentListItem_Formation>
    {
        #region 바인딩
        enum Dropdowns
        {
        ...
        internal void ShowItem(AgentTemplate template)
        {
            var item = _agentListItems.Find(x => x.template == template);

            if (item != null)
            {
                (item as UIAgentListItem_Formation).Show();
            }
        }

        internal void HideItem(AgentTemplate template)
        {
            var item = _agentListItems.Find(x => x.template == template);

            if (item != null)
            {
                (item as UIAgentListItem_Formation).Hide();
            }
        }
